...
def upgrade() -> None:
    ...
    if bind.dialect.name == 'postgresql':
        # Safely add new values to paymentstatus enum if they don't exist
        new_values = ['DRAFT', 'PARTIALLY_PAID', 'VOID']
        for val in new_values:
            op.execute(f"ALTER TYPE paymentstatus ADD VALUE IF NOT EXISTS '{val}'")
    ...
    # Cast invoices.status to paymentstatus enum on Postgres
    if bind.dialect.name == 'postgresql':
        op.execute("ALTER TABLE invoices ALTER COLUMN status TYPE paymentstatus USING status::paymentstatus")
    ...
    # Cast payments columns to enums on Postgres
    if bind.dialect.name == 'postgresql':
        op.execute("ALTER TABLE payments ALTER COLUMN status TYPE paymentstatus USING status::paymentstatus")
        # Handle paymentmethod enum creation if not exists
        op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'paymentmethod') THEN CREATE TYPE paymentmethod AS ENUM ('CASH', 'UPI', 'CARD', 'NET_BANKING', 'INSURANCE', 'OTHER'); END IF; END $$")
        # Drop default before casting to avoid type mismatch on the default value itself
        op.execute("ALTER TABLE payments ALTER COLUMN payment_method DROP DEFAULT")
        op.execute("ALTER TABLE payments ALTER COLUMN payment_method TYPE paymentmethod USING payment_method::paymentmethod")
        op.execute("ALTER TABLE payments ALTER COLUMN payment_method SET DEFAULT 'CASH'::paymentmethod")
