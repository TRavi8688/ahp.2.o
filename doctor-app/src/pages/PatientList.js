...
export default function PatientList() {
    ...
    React.useEffect(() => {
        const fetchPatients = async () => {
            try {
                const response = await fetch('http://localhost:8000/doctor/my-patients', {
                    headers: {
    ...
    React.useEffect(() => {
        ...
        const fetchDetail = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:8000/doctor/patient/${selectedPatientId}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    return (
        <Box sx={{ display: 'flex', height: 'calc(100vh - 120px)', gap: 3, mx: 'auto' }}>
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', bgcolor: 'white', border: '1px solid #e5e7eb', borderRadius: 2 }}>

                {/* Search Header */}
                <Box sx={{ p: 2, borderBottom: '1px solid #e5e7eb', display: 'flex', gap: 2 }}>
                    <Box sx={{
                        flex: 1,
                        display: 'flex',
                        alignItems: 'center',
            ...
            <Box sx={{ width: 380, flexShrink: 0, bgcolor: 'white', border: '1px solid #e5e7eb', borderRadius: 2, display: 'flex', flexDirection: 'column' }}>
                {!selectedPatientData ? (
